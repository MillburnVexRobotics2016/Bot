...
task autonomous()
{
	...
	//testing timing, eg. how long it takes to do X
...
task driveM()
{
	...
	while(true)
	{
		if(vexRT[Ch2] > deadzone || vexRT[Ch2] < -(deadzone) || vexRT[Ch1] > deadzone || vexRT[Ch1] < -(deadzone)){
	...
	//claw and arm rotation
	  ...
	  //close
		...
		if(vexRT[Btn7U] || vexRT[Btn6U]) {
			...
			motor[xLeft] = xLeftS;
		} else if(vexRT[Btn7D] || vexRT[Btn6D]) {
			motor[xRight] = xRightS;
			motor[xLeft] = -xLeftS;
		} else if(vexRT[Btn7L]) {
			motor[xLeft] = xLeftS;
